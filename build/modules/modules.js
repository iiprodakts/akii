...
function Requery(sandbox) {
	this.sb = sandbox;this.query = null;
}
...
Messenger.prototype.formatRQ = function (data) {
	...
	if (data.method) {
		...
		if (data.method === 'post') {
			...
			this.send({ method: 'post', data: data });
		} else {

			this.send({ method: 'get', data: data });
		}
	} else {
		...
		checkout: {
			...
			functions: {
				...
				delivery: function delivery(tool, els) {
					...
					var methods = [{ label: 'Instore Serve', value: 'instore', run: tool.events.save }, { label: 'Takeaway', value: 'takeaway', run: tool.events.save }, { label: 'Home delivery', value: 'home', run: tool.events.reveal }];
					var revealContent = {
				...
				payment: function payment(tool, els) {
					...
					var methods = [{ label: 'Cashpay', value: 'cashpay', run: tool.events.save }, { label: 'Paypal', value: 'paypal', run: tool.events.save }, { label: 'Visa', value: 'visa', run: tool.events.reveal }];
