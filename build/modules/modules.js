...
Register.prototype.fail = function (data) {
  var sb = this.sb; //   var resEl = sb.sb_getChildById('#response')
...
Login.prototype.getLoginData = function () {
  ...
  var url = 'https://smarfoapi.herokuapp.com/smarfo/login'; //    var url = 'http://localhost:3000/smarfo/login'
...
Login.prototype.fail = function (data) {
  var sb = this.sb;
  var resEl = sb.sb_getChildById('#response');
  resEl.innerHTML = 'An error occured,request could not be completed,please try again'; // this.emit({type: 'log-error',data: data})
};
...
Logout.prototype.setUpLogOut = function () {
  var sb = this.sb; // var loginT = sb.sb_getChildById('#login')
...
Slider.prototype.init = function () {
  ...
  addControlsHandler(); // Event Handlers functions
  // Slider handlers
...
Requery.prototype.parseQuery = function () {
  ...
  if (query) {
    ...
    console.log(query);
    this.query = query; //query.success = 'success'

    this.unsetQuery(this.getCurrentPage()); //  this.done(query)
  } else {
...
Render.prototype.handleTakeQueryData = function (data) {
  ...
  if (data instanceof Object && typeof data !== 'string') {
    this.emit({
      type: 'render-component-' + data.page,
      data: data
    });
  } else {
    this.emit({
      type: 'render-component-' + data,
      data: data
    });
  }
};
...
Component.prototype.listens = function () {
  var _sb$sb_notifyListen;

  var sb = this.sb;
  var name = 'render-component-' + this.componentname;
  sb.sb_notifyListen((_sb$sb_notifyListen = {
    'request-query-success': this.handleRQS.bind(this)
  }, _defineProperty(_sb$sb_notifyListen, name, this.handleComponentRender.bind(this)), _defineProperty(_sb$sb_notifyListen, 'take-component-name', this.handleTakeComponentName.bind(this)), _defineProperty(_sb$sb_notifyListen, 'component-resource-creation-done', this.handleComponentCreationDone.bind(this)), _sb$sb_notifyListen), sb.moduleMeta.moduleId, sb.moduleMeta.modInstId);
};
  ...
  var tools = {
    ...
    cart: {
      events: {
        ...
        startOrder: function startOrder(tool, ev) {
          var sb = this.sb;
          sb.sb_preventNormal(ev);
      ...
      functions: {
        ...
        calculate: function calculate(calcs) {
          var totalPrice = 0;
    ...
    checkout: {
      events: {
        loadNext: function loadNext(tool, els, ev) {
          var sb = that.sb;
        ...
        save: function save(tool, els, stage, ev) {
          var sb = that.sb;
        ...
        reveal: function reveal(tool, els, _reveal, append, ev) {
          var sb = that.sb;

          if (append.contains(_reveal)) {
            console.log('Main contains reveal');
            var computedStyle = document.defaultView.getComputedStyle(_reveal, null);
            console.log(computedStyle.display);

            if (computedStyle.display === 'block') {
              _reveal.style.display = 'none';
            } else {
              _reveal.style.display = 'block';
            }
          } else {
            console.log('Main does not reveal');
            sb.sb_addChild(append, _reveal);
          }
        },
        captureAddress: function captureAddress(tool, els, stage, ev) {
          var sb = that.sb;
          sb.sb_stopEventBubble(ev);
          sb.sb_preventNormal(ev);
        ...
        capturePayment: function capturePayment(tool, els, stage, ev) {
          var sb = that.sb;
          sb.sb_stopEventBubble(ev);
          sb.sb_preventNormal(ev);
        ...
        orderPlaced: function orderPlaced(data) {
          var sb = that.sb;
          ...
          localStorage.setItem('orderno', sb.sb_jsToJson({
            orderno: data.order.order_no
          }));
        }
      },
      functions: {
        cart: function cart(evt) {
          var sb = that.sb;
          window.location.href = 'kart.html';
        },
        delivery: function delivery(tool, els) {
          var sb = that.sb;
          console.log('The delivery function runs');
          var methods = [{
            label: 'Instore Serve',
            value: 'instore',
            run: tool.events.save
          }, {
            label: 'Takeaway',
            value: 'takeaway',
            run: tool.events.save
          ...
          for (var i = 0; i < methods.length; i++) {
            ...
            sb.sb_addProperty(updatorLabelSp, 'class', 'form__radio__button'); //    sb.sb_addProperty(uinputId,'type','hidden')
            ...
            sb.sb_insertInner(idCont, methods[i].label); //    sb.sb_insertInner(priceCont,'R854')
            ...
            sb.sb_addChild(uli, idCont); //    sb.sb_addChild(uli,priceCont)
          ...
          tool.identifiers.steps['delivery'].called = true; // tool.identifiers.current = 'delivery'
        },
        payment: function payment(tool, els) {
          var _revealContent;
          ...
          var revealContent = (_revealContent = {}, _defineProperty(_revealContent, 'card holder', {
            control: {
              name: 'input',
              type: 'text',
          ...
          }), _defineProperty(_revealContent, 'card number', {
            control: {
              name: 'input',
              type: 'text',
          ...
          }), _defineProperty(_revealContent, "expiry", {
            control: {
              name: 'input',
              type: 'text',
              placeholder: '05/2020',
          ...
          }), _defineProperty(_revealContent, "cvv", {
            control: {
              name: 'input',
              type: 'text',
              placeholder: '000',
          ...
          }), _defineProperty(_revealContent, "save", {
            control: {
              name: 'input',
              type: 'submit',
              placeholder: 'Postal code',
          ...
          for (item in revealContent) {
            ...
            sb.sb_addProperty(input, 'placeholder', revealContent[item].control.placeholder);
            sb.sb_addProperty(input, 'value', revealContent[item].control.text);
            sb.sb_addProperty(input, 'name', revealContent[item].control.id); // sb.sb_addProperty(input,'name',revealContent[item].control.id)
          ...
          tool.identifiers.steps['payment'].called = true; // tool.identifiers.current = 'delivery'
        },
        confirm: function confirm(tool, els) {
          ...
          var paymentD = sb.sb_createElement('section'); // sb.sb_addChild(els.main,lst)

          for (var p = 0; p < cart.length; p++) {
            var li = sb.sb_createElement('li');
            sb.sb_addProperty(li, 'class', 'hr-size-fl-xx-bg cursor-pointer fd-font-x-tn mg-bottom-fd-xxx-tn pd-left-fl-bt bg-light pos-rel d-block bd-bottom-fd-secondary-xx-bt pd-bottom-fd-tn pd-top-fd-tn'); // sb.sb_addProperty(li,'data-navigata-page','detail')
            ...
            var input = sb.sb_createElement('span');
            var rm = sb.sb_createElement('span');
            ...
            sb.sb_addProperty(img, 'class', 'hr-size-fl-xx-md mg-left-fl-xx-sm bd-rad-bt pos-rel'); // sb.sb_addProperty(fm,'class','hr-fl-size-xx-bg mg-top-fl-x-tn mg-bottom-fd-sm pos-rel')
            ...
            sb.sb_addProperty(rm, 'class', ' font-fd-x-tn text-align-center fg-secondary font-wt-bolder pd-fd-xxx-tn d-inline-block hr-size-fd-xx-sm pos-rel right-offset-fl-bt top-offset-fl-xxx-tn '); // sb.sb_addProperty(input,'placeholder','0')
            ...
            sb.sb_insertInner(sp2, cart[p].productName); // sb.sb_insertInner(minus,'-')

            sb.sb_insertInner(input, cart[p].productQty); // sb.sb_insertInner(add,'+')

            sb.sb_insertInner(priceSp, 'R' + cart[p].productPrice);
            sb.sb_addChild(sp, img); // sb.sb_addChild(sp3,minus)
            // sb.sb_addChild(sp3,input)
            // sb.sb_addChild(sp3,add)

            sb.sb_addChild(sp3, rm); // sb.sb_addChild(sp3,fm)
          ...
          sb.sb_addChild(stageContent, paymentD); // sb.sb_addChild(stageContent,optionsForm)
        ...
        order: function order(tool, els) {
          ...
          var url = 'https://smarfoapi.herokuapp.com/smarfo/order'; //var url = 'http://localhost:3000/smarfo/order'
...
Component.prototype.kart = function (data) {
  ...
  if (_typeof(cartstate.empty) !== undefined) {
    if (cartstate.empty === true) {
      ...
      sb.sb_addProperty(customise, 'class', 'link d-block pos-fd hr-size-fl-xx-bg bottom-offset-0 bg-light pd-top-fd-xxx-sm pd-bottom-fd-xx-bt z-index'); // sb.sb_addProperty(customise,'href','checkout.html')
      ...
      for (var p = 0; p < cartstate.cart.length; p++) {
        var li = sb.sb_createElement('li');
        sb.sb_addProperty(li, 'class', 'hr-size-fl-xx-bg cursor-pointer fd-font-x-tn mg-bottom-fd-tn pd-left-fl-bt bg-light pos-rel d-block bd-bottom-fd-secondary-xx-bt pd-bottom-fd-tn pd-top-fd-tn'); // sb.sb_addProperty(li,'data-navigata-page','detail')
        // sb.sb_addProperty(li,'data-navigata-data','[{data: '+itemData.item_id+',page: detail,endpoint: /detail}]')
        ...
        var _img = sb.sb_createElement('img');
        ...
        sb.sb_addProperty(inputName, 'value', cartstate.cart[p].productName); // sb.sb_addProperty(input,'type','number')
        ...
        sb.sb_addProperty(_img, 'src', 'img/fish.jpg');
        sb.sb_addProperty(_img, 'class', 'hr-size-fl-xx-md bd-rad-bt pos-rel');
        sb.sb_addProperty(fm, 'class', 'hr-fl-size-xx-bg mg-top-fl-x-tn mg-bottom-fd-sm pos-rel'); // sb.sb_addProperty(fm,'id',ct+'-form')

        sb.sb_addProperty(minus, 'class', 'hr-size-fl-xxx-sm cursor-pointer float-left vt-size-fd-bt d-inline-block bg-light bd-bottom-left-rad-fd-xx-bt bd-top-left-rad-fd-xx-bt bd-fd-secondary-xx-bt pd-fd-xx-tn text-align-center  font-fd-xx-tn font-wt-bolder  pos-rel'); // sb.sb_addProperty(minus,'id',ct)
        // sb.sb_addEvent(minus,'click',this.removeFromCart.bind(this))

        sb.sb_addProperty(add, 'class', 'hr-size-fl-xxx-sm cursor-pointer float-left vt-size-fd-bt d-inline-block  bg-light bd-bottom-right-rad-fd-xx-bt bd-top-right-rad-fd-xx-bt bd-fd-secondary-xx-bt pd-fd-xx-tn text-align-center font-wt-bolder  font-fd-xx-tn pos-rel'); // sb.sb_addProperty(add,'id',ct)
        // sb.sb_addEvent(add,'click',this.addToCart.bind(this))

        sb.sb_addProperty(input, 'class', 'vt-size-fd-bt float-left bg-secondary font-fd-xx-tn text-align-center fg-light font-wt-bolder d-inline-block hr-size-fd-md pd-fd-xx-tn bd-fd-secondary-xx-bt ');
        sb.sb_addProperty(rm, 'class', ' font-fd-tn text-align-center fg-red font-wt-bolder pd-fd-xxx-tn bd-rad-xxx-tn d-inline-block hr-size-fd-xx-sm pos-abs right-offset-fl-x-bt top-offset-0'); // sb.sb_addProperty(input,'placeholder','0')
        // sb.sb_addEvent(input,'input',this.updateCart.bind(this))
        ...
        sb.sb_addChild(sp, _img); // sb.sb_addChild(sp3,minus)
        // sb.sb_addChild(sp3,input)
        // sb.sb_addChild(sp3,add)

        sb.sb_addChild(sp3, rm);
        sb.sb_addChild(sp3, fm); // sb.sb_addChild(small,smallSp)
...
Component.prototype.menulist = function (data) {
  ...
  var menuUrl = 'https://smarfoapi.herokuapp.com/smarfo/menu'; //var url = 'http://localhost:3000'+data.endpoint+'/'+ data.data   
  ...
  function menuSuccess(data) {
    ...
    var sb = this.sb;
    var menu = sb.sb_jsonToJs(data);
    var menuList = menu.categories;
    var modalList = sb.sb_createElement('ul');
    sb.sb_addProperty(modalList, 'class', 'list list--hr list--none'); //  var ing = sb.sb_createElement('div')
...
Component.prototype.dashboard = function (data) {
  ...
  var url = 'https://smarfoapi.herokuapp.com/smarfo/menu'; // var url = 'http://localhost:3000/smarfo/menu' 
...
Component.prototype.detail = function (data) {
  var sb = this.sb; // var catlist = sb.sb_createElement('article')

  console.log(data.data);
  var url = 'https://smarfoapi.herokuapp.com/smarfo' + data.endpoint + '/Starters/' + data.data; // var url = 'http://localhost:3000/smarfo'+data.endpoint+'/Starters/'+data.data
  // var sdata = {
  // 	menuitem: "Starters",
  ...
  function success(data) {
    ...
    sb.sb_addChild(actions, actionsCustomise);
    sb.sb_addChild(actions, actionsCart); // sb.sb_addChild(actions,actionsCustomise)
    ...
    sb.sb_addChild(actionsCustomise, customise);
    sb.sb_addChild(customise, customiseBtn);
    sb.sb_addChild(productP, actions); // for(p in data){
    ...
    sb.sb_insertInner(desc, data.detail.description); // Modal
    ...
    for (v in data.detail.variants) {
      ...
      if (v.trim() === "drinks") {
        sb.sb_insertInner(sp, 'Drink Choice');
      } else {
        sb.sb_insertInner(sp, v);
      }
  ...
  function fail(data) {
    ...
    console.log(data); // this.emit({type: 'stop-preloader',data: data})
  }
};
...
Component.prototype.bargain = function (data) {
  ...
  console.log(data.data);
  var url = 'https://smarfoapi.herokuapp.com/smarfo/bargain'; //var url = 'http://localhost:3000/smarfo/bargain'
  // var sdata = {
  // 	menuitem: "Starters",
  // 	item: data.data.item_id
  // }
  ...
  function success(data) {
    ...
    sb.sb_addProperty(bPrice, 'class', 'pos-abs d-block left-offset-fl-xx-sm top-offset-vh-xx-tn font-fd-tn'); // sb.sb_insertInner(bTitle,'Grab our daily bargain')
    ...
    sb.sb_addChild(bargain, bA); // sb.sb_addChild(bargain,bTag)
...
Component.prototype.manage = function (data) {
  ...
  console.log(data.data);
  var url = 'http://localhost:3000/smarfo/bargain'; // var sdata = {
  // 	menuitem: "Starters",
  // 	item: data.data.item_id
  // }
...
Component.prototype.orderstatus = function (data) {
  var sb = this.sb;
  var url = 'https://smarfoapi.herokuapp.com/smarfo/order/track'; // var url = 'http://localhost:3000/smarfo/order/track'
  ...
  function success(data) {
    ...
    if (trackresponse.status) {
      ...
      sb.sb_addProperty(completedStageLbC, 'class', 'font-fd-xx-tn'); // sb.sb_addProperty(placedStageLb,'class','d-inline-block mg-bottom-fd-md')
      ...
      sb.sb_addProperty(trackEst, 'class', 'd-block bg-dark-lta pd-top-fd-tn hr-size-fl-xx-bg vt-size-fd-xxx-tn'); // sb.sb_addProperty(success,'class','d-inline-block font-fd-xxx-tn fg-dark-lt')
      ...
      sb.sb_insertInner(completedStageLbC, 'Your order has been successfully completed'); // sb.sb_insertInner(completed,'&#10003;')
      ...
      sb.sb_insertInner(trackEstOdNm, '#' + trackresponse.orderstatus.orderno); // sb.sb_insertInner(sb.view,'')
      ...
      sb.sb_addChild(stages, stagesBgTr); // sb.sb_addChild(stages,stagesBgF)

      sb.sb_addChild(stages, placedStage); // sb.sb_addChild(placedStageBtn,completed)
      ...
      sb.sb_addChild(stages, confirmStage);
      sb.sb_addChild(confirmStage, confirmStageBtn); // sb.sb_addChild(confirmStageBtn,activeStage)
      ...
      sb.sb_addChild(received, stagesLabels); // sb.sb_addChild(sb.view,received)
        ...
        case 'placed':
          {
            ...
            sb.sb_addChild(confirmStageBtn, sb.sb_copyDeep(completed)); // sb.sb_addProperty(placedStageLbL,'class','font-fd-x-tn mg-bottom-fd-xxx-tn fg-dark')
        ...
        default:
          {
            ...
            sb.sb_addChild(progressStageBtn, sb.sb_copyDeep(completed)); // sb.sb_addProperty(placedStageLbL,'class','font-fd-x-tn mg-bottom-fd-xxx-tn fg-dark')
            // sb.sb_addProperty(confirmStageLbL,'class','font-fd-x-tn mg-bottom-fd-xxx-tn fg-dark')
            // sb.sb_addProperty(progressStageLbL,'class','font-fd-x-tn mg-bottom-fd-xxx-tn fg-dark')
...
Modal.prototype.getParent = function (options) {
  ...
  sb.sb_addProperty(parent, 'data-modal', options.id); //    if(options.opacity){
...
Modal.prototype.createModalBody = function (options) {
  ...
  sb.sb_addProperty(body, 'class', options.class); //    sb.sb_addChild(body,options.content)

  sb.sb_addChild(body, options.body); //    switch(options.size){
  //    	  case 1:{
  //    	  }
  //    }
  ...
  if (data.length > 0) {
    for (var modal = 0; modal < data.length; modal++) {
      ...
      if (data[modal].foot.foot) {
        var foot = this.createModalFoot(data[modal].foot); // sb.sb_addChild(parent,foot.foot)
...
Modal.prototype.openModal = function (data, ev) {
  ...
  console.log(computedStyle.display); // var classList = sb.sb_getClasses(data)
  // sb.sb_toggleClass(classList,'d-none')
...
Modal.prototype.closeModal = function (data, targ, ev) {
  var sb = this.sb;
  console.log('The code gets at this point'); // console.log(ev)
  // console.log('data')
  // console.log(data)
  ...
  if (ev.target === data) {
    console.log('The events is initiated by the window'); // var classList = sb.sb_getClasses(data)
...
function Cart(sandbox) {
  this.sb = sandbox;
}

Cart.prototype.init = function () {
  var sb = this.sb; // console.log(this)
  ...
  if (localStorage.cart) {
    ...
    if (products.length > 0) {
      for (var p = 0; p < products.length; p++) {
        ...
        console.log(_typeof(products[p].productId));
        console.log('The productId inside the add product');
        console.log(product.productId);
        console.log(_typeof(product.productId));
        console.log(products[p].productId);

        if (products[p].productId === product.productId) {
          console.log('The productIds match here'); // console.log(product.productQty)
...
Navigata.prototype.startNavigata = function () {
  var sb = this.sb; //  console.log('The value of this')
  //  console.log(this)
...
Navigata.prototype.addLinkEvents = function (lk, page, pgData) {
  ...
  console.log(page);
  console.log('Data');
  console.log(pgData); //  console.log('The value of lk')
  //  console.log(lk)
...
Navigata.prototype.openLink = function (page, pgData) {
  ...
  // console.log(this)
  var sb = this.sb;
  localStorage.setItem(page, sb.sb_jsToJson(pgData)); // console.log('LocalStorage data')
...
Aka.prototype.create = function (name, props, ops) {
  console.log('Create');
  console.log(props);
  var sb = this.sb;
...
Aka.prototype.createChildren = function (root, children) {
  var sb = this.sb;
  var descends = [];
...
Aka.prototype.addChildren = function (parent, children) {
  var sb = this.sb;
...
Aka.prototype.addOps = function (el, ops) {
  var sb = this.sb;
  console.log('ADD OPS');
  console.log(ops);
  console.log(el);
...
List.prototype.listens = function () {
  ...
  sb.sb_notifyListen({
    'create-list': this.handleCreateForm.bind(this)
  }, sb.moduleMeta.moduleId, sb.moduleMeta.modInstId);
};
